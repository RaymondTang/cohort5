...
# Using Pydantic BaseModel for automatic validation and serialization
class BaseMoonCake(BaseModel):
    # Model configuration for Pydantic v2
    model_config = ConfigDict(
        # Allow extra fields to be ignored
        extra='ignore',
        # Validate assignment to fields after model creation
    ...
    name: str = Field(..., description="The name of the mooncake")  # Required: the name of the mooncake
    price: float = Field(..., gt=0, description="Price must be greater than 0")  # Required: how much it costs, must be positive
    size: str = Field(default='regular', description="Size of the mooncake")  # Optional: size defaults to 'regular' if not specified
    is_vegetarian: bool = Field(default=False, description="Whether the mooncake is vegetarian")  # Optional: defaults to False (not vegetarian)
    discount: float = Field(default=0.0, ge=0.0, le=1.0, description="Discount percentage (0.0 = no discount, 1.0 = 100% off)")  # Optional: discount percentage, must be between 0 and 1
    expiry_date: date = Field(default_factory=lambda: date.today() + timedelta(days=365), description="Expiry date of the mooncake")  # Optional: expires in 1 year from today
...
class VegMoonCake(BaseMoonCake):
    ...
    is_vegetarian: bool = Field(default=True, description="Vegetarian mooncakes are always vegetarian")  # This overrides the parent class default of False
...
class LotusSeedMoonCake(BaseMoonCake):
    ...
    egg_yolks: int = Field(default=1, ge=0, le=10, description="Number of egg yolks in the mooncake")  # How many egg yolks are in this mooncake (defaults to 1), must be between 0 and 10


# ShoppingCart class to manage a collection of mooncakes
# This is a regular class (not a Pydantic model) because it has complex methods
class ShoppingCart:
...
for i in range(5):  # Loop 5 times (i = 0, 1, 2, 3, 4)
    ...
    # Pydantic models can be created from dictionaries using model_validate
    cake = LotusSeedMoonCake.model_validate(cake_data)
...
for i in range(5):  # Loop 5 times (i = 0, 1, 2, 3, 4)
    ...
    # Pydantic models can be created from dictionaries using model_validate
    cake = VegMoonCake.model_validate(cake_data)
...
try:
    # This will work fine
    valid_cake = LotusSeedMoonCake(name="Test", price=15.0, egg_yolks=2)
    print(f"Valid cake created: {valid_cake.name}")
    
    # This will raise a validation error
    invalid_cake = LotusSeedMoonCake(name="Test", price=-5.0, egg_yolks=15)
